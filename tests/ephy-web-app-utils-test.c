...
test_web_app_lifetime (void)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (test_web_app); i++) {
    ...
    g_assert_true (g_file_test (desktop_link, G_FILE_TEST_EXISTS) == FALSE);
    g_assert_true (g_file_test (desktop_link, G_FILE_TEST_IS_SYMLINK) == FALSE);
    g_assert_true (g_file_test (profile_dir, G_FILE_TEST_EXISTS) == FALSE);
    g_assert_true (ephy_web_application_exists (id) == FALSE);
