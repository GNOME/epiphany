...
#ifndef EPHY_AUTOFILL_FIELD_H
#define EPHY_AUTOFILL_FIELD_H
 ...
 *
 * In this case @EPHY_AUTOFILL_FIELD_COUNTRY is used for matching - as in ephy-autofill-matchers.c.
 * And is also used in ephy_autofill_storage_get() where it returns some default value
 * representing some of the two specific values:
 *  1. @EPHY_AUTOFILL_FIELD_COUNTRY_NAME
 *  2. @EPHY_AUTOFILL_FIELD_COUNTRY_CODE
 * However, specific values are required for example in ephy_autofill_storage_set()
 * The main reason for this is that only specific values are stored
 * and general values are used to simplify some operations: retrieval, matching, etc...
 **/
typedef enum
{
  ...
  EPHY_AUTOFILL_FIELD_PERSONAL = (EPHY_AUTOFILL_FIELD_FIRSTNAME | EPHY_AUTOFILL_FIELD_LASTNAME |
                                  EPHY_AUTOFILL_FIELD_FULLNAME | EPHY_AUTOFILL_FIELD_USERNAME |
                                  EPHY_AUTOFILL_FIELD_EMAIL | EPHY_AUTOFILL_FIELD_PHONE |
                                  EPHY_AUTOFILL_FIELD_STREET_ADDRESS | EPHY_AUTOFILL_FIELD_COUNTRY_CODE |
                                  EPHY_AUTOFILL_FIELD_COUNTRY_NAME | EPHY_AUTOFILL_FIELD_ORGANIZATION |
                                  EPHY_AUTOFILL_FIELD_POSTAL_CODE | EPHY_AUTOFILL_FIELD_COUNTRY |
                                  EPHY_AUTOFILL_FIELD_STATE | EPHY_AUTOFILL_FIELD_CITY),

  EPHY_AUTOFILL_FIELD_CARD = (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH_MM | EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH_M |
                              EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH | EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR_YYYY |
                              EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR_YY | EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR |
                              EPHY_AUTOFILL_FIELD_CARD_EXPDATE | EPHY_AUTOFILL_FIELD_NAME_ON_CARD |
                              EPHY_AUTOFILL_FIELD_CARD_NUMBER | EPHY_AUTOFILL_FIELD_CARD_TYPE_CODE |
                              EPHY_AUTOFILL_FIELD_CARD_TYPE_NAME | EPHY_AUTOFILL_FIELD_CARD_TYPE),

  EPHY_AUTOFILL_FIELD_GENERAL = (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH | EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR |
                                 EPHY_AUTOFILL_FIELD_CARD_TYPE | EPHY_AUTOFILL_FIELD_COUNTRY),
