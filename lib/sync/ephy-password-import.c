...
static char *
get_libsecret_phrase (gboolean is_chrome)
{
                                  ...
                                  GError              **error)
{
  ...
  const char *statement_str = "SELECT origin_url, action_url, username_element, username_value, password_element, password_value FROM logins";
  ...
  while (ephy_sqlite_statement_step (statement, &my_error)) {
    ...
    if (!secret_phrase) {
      if (memcmp (password, "v11", 3) == 0) {
        secret_phrase = get_libsecret_phrase (is_chrome);
