                           ...
                           QueryAsyncData *data)
{
  ...
  for (GList *l = matches; l && l->data; l = l->next) {
    ...
    gint64 created = secret_retrievable_get_created (retrievable);
    gint64 modified = secret_retrievable_get_modified (retrievable);
    const char *password;
    gint64 server_time_modified;
    EphyPasswordRecord *record;
    ...
    value = secret_retrievable_retrieve_secret_sync (retrievable, NULL, &error);
    if (!value) {
