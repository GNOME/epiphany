 ...
 **/
guint
ephy_label_get_mnemonic_keyval (EphyLabel *label)
{
  g_return_val_if_fail (EPHY_IS_LABEL (label), GDK_VoidSymbol);

  return label->mnemonic_keyval;
}

static void
ephy_label_set_text_internal (EphyLabel *label,
			     gchar    *str)
{
  g_free (label->text);
  ...
  ephy_label_select_region_index (label, 0, 0);
}

static void
ephy_label_set_label_internal (EphyLabel *label,
			      gchar    *str)
{
  g_free (label->label);
...
static void
ephy_label_set_use_markup_internal (EphyLabel *label,
				   gboolean  val)
{
  val = val != FALSE;
...
static void
ephy_label_set_use_underline_internal (EphyLabel *label,
				      gboolean val)
{
  val = val != FALSE;
...
static void
ephy_label_set_attributes_internal (EphyLabel      *label,
				   PangoAttrList *attrs)
{
  if (attrs)
				   ...
				   const gchar *str)
{
  ...
  *pattern_dest = 0;
  ...
  ephy_label_set_text_internal (label, new_str);
  ephy_label_set_pattern_internal (label, pattern);
...
guint      
ephy_label_parse_uline (EphyLabel    *label,
		       const gchar *str)
{
  guint keyval;
  guint orig_keyval;
  ...
  ephy_label_setup_mnemonic (label, orig_keyval);
...
/**
 * ephy_label_set_text_with_mnemonic:
 * @label: a #EphyLabel
 * @str: a string
 * 
...
static void
get_layout_index (EphyLabel *label,
                  gint      x,
                  gint      y,
                  gint     *index)
{
...
static void
ephy_label_select_word (EphyLabel *label)
{
  gint min, max;
  
  gint start_index = ephy_label_move_backward_word (label, label->select_info->selection_end);
