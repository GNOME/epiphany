...
static void
response_dir_cb (GDBusConnection *connection,
                 const char      *sender_name,
                 const char      *object_path,
                 const char      *interface_name,
                 const char      *signal_name,
                 ...
                 gpointer         user_data)
{
  ...
  if (response == 0)
    ...
    g_task_return_new_error (task, G_IO_ERROR, G_IO_ERROR_FAILED, "Portal failed to open directory");
}

static void
open_dir_complete_cb (GObject      *source,
                      GAsyncResult *result,
                      gpointer      user_data)
{
  GDBusProxy *proxy = G_DBUS_PROXY (source);
  ...
  if (!return_value) {
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
      g_warning ("Failed to open directory via portal: %s", error->message);
    g_task_return_error (task, error);
    goto out;
  }
...
static void
portal_dir_proxy_created_cb (GObject      *source,
                             GAsyncResult *result,
                             gpointer      user_data)
{
  GTask *task;
  ...
  if (!proxy) {
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
      g_warning ("Failed to create D-Bus proxy for OpenDirectory portal: %s", error->message);
    g_task_return_error (task, error);
    close (fd);
    return;
  }
  ...
  g_dbus_proxy_call_with_unix_fd_list (proxy,
                                       ...
                                       fd_list,
                                       g_task_get_cancellable (task),
                                       open_dir_complete_cb,
                                       task);
  g_object_unref (fd_list);
}

void
ephy_open_directory_via_flatpak_portal (const char          *path,
                                        GCancellable        *cancellable,
                                        GAsyncReadyCallback  callback,
                                        gpointer             user_data)
{
  ...
  char *parent = g_file_get_path (file);

  fd = open (parent, O_RDONLY | O_CLOEXEC);
  if (fd == -1) {
    g_warning ("Failed to open %s: %s", path, g_strerror (errno));
    return;
  }
  ...
  g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION,
                            ...
                            "org.freedesktop.portal.OpenURI",
                            cancellable,
                            portal_dir_proxy_created_cb,
                            task);
}

gboolean
ephy_open_dir_via_flatpak_portal_finish (GAsyncResult  *result,
                                         GError       **error)
{
  gboolean ret;
