...
ephy_location_controller_constructed (GObject *object)
{
  ...
  GtkWidget *reader_mode = ephy_location_entry_get_reader_mode_widget (EPHY_LOCATION_ENTRY (controller->title_widget));
  g_signal_connect (G_OBJECT (reader_mode), "button-press-event", G_CALLBACK (reader_mode_button_press_event_cb), controller);

  GtkWidget *entry = ephy_location_entry_get_entry (EPHY_LOCATION_ENTRY (controller->title_widget));
