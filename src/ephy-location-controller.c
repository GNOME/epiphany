...
{
  const char *address;
  DzlSuggestionEntry *entry = ephy_location_entry_get_entry (EPHY_LOCATION_ENTRY (widget));
  GListModel *model;
...
ephy_location_controller_constructed (GObject *object)
{
  ...
  g_signal_connect (G_OBJECT (EPHY_LOCATION_ENTRY (controller->title_widget)), "user-changed", G_CALLBACK (user_changed_cb), controller);
