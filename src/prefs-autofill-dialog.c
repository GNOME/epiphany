...
clear_personal_data (PrefsAutofillDialog *dialog)
{
  ...
  gtk_combo_box_set_active_id (GTK_COMBO_BOX (dialog->country_combo_box_text),
                               "US");
...
{
  gtk_combo_box_set_active_id (GTK_COMBO_BOX (dialog->expdate_year_combo_box_text),
                               "19");
  gtk_combo_box_set_active_id (GTK_COMBO_BOX (dialog->expdate_month_combo_box_text),
                               "1");
  gtk_combo_box_set_active_id (GTK_COMBO_BOX (dialog->card_type_combo_box_text),
                               "A");
                ...
                const char *value)
{
  if (is_empty_value (value))
    ephy_autofill_storage_delete (field, NULL, NULL);
  else
    ephy_autofill_storage_set (field, value, NULL, NULL);
}
...
save_personal_data (PrefsAutofillDialog *dialog)
{
  ...
  char *fullname = (is_empty_value (firstname) || is_empty_value (lastname))
    ? NULL
    : g_strjoin (" ", firstname, lastname, NULL);
  ...
  save_or_delete (EPHY_AUTOFILL_FIELD_USERNAME,
                  gtk_entry_get_text (dialog->username_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_EMAIL,
                  gtk_entry_get_text (dialog->email_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_PHONE,
                  gtk_entry_get_text (dialog->phone_entry));

  save_or_delete (EPHY_AUTOFILL_FIELD_STREET_ADDRESS,
                  gtk_entry_get_text (dialog->street_address_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_ORGANIZATION,
                  gtk_entry_get_text (dialog->organization_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_POSTAL_CODE,
                  gtk_entry_get_text (dialog->postal_code_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_STATE,
                  gtk_entry_get_text (dialog->state_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_CITY,
                  gtk_entry_get_text (dialog->city_entry));

  if (is_empty_value (fullname))
    ephy_autofill_storage_delete (EPHY_AUTOFILL_FIELD_FULLNAME, NULL, NULL);
  else
    ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_FULLNAME,
                               fullname,
                               prefs_autofill_utils_set_free_cb,
                               fullname);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_COUNTRY_CODE,
                             country_code,
                             NULL,
                             NULL);
  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_COUNTRY_NAME,
                             country_name,
                             prefs_autofill_utils_set_free_cb,
                             country_name);
}
...
save_card_data (PrefsAutofillDialog *dialog)
{
  ...
  char *expdate_mm_yyyy = (year_yy == NULL || month_mm == NULL)
    ? NULL
    : g_strjoin ("/", month_mm, year_yy, NULL);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_EXPDATE,
                             expdate_mm_yyyy,
                             prefs_autofill_utils_set_free_cb,
                             expdate_mm_yyyy);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH_M,
                             month_m,
                             NULL,
                             NULL);
  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR_YY,
                             year_yy,
                             NULL,
                             NULL);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR_YYYY,
                             year_yyyy,
                             prefs_autofill_utils_set_free_cb,
                             year_yyyy);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH_MM,
                             month_mm,
                             prefs_autofill_utils_set_free_cb,
                             month_mm);

  save_or_delete (EPHY_AUTOFILL_FIELD_NAME_ON_CARD,
                  gtk_entry_get_text (dialog->name_on_card_entry));
  save_or_delete (EPHY_AUTOFILL_FIELD_CARD_NUMBER,
                  gtk_entry_get_text (dialog->card_number_entry));

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_TYPE_CODE,
                             prefs_autofill_utils_get_active_id (dialog->card_type_combo_box_text),
                             NULL,
                             NULL);

  ephy_autofill_storage_set (EPHY_AUTOFILL_FIELD_CARD_TYPE_NAME,
                             card_type_name,
                             prefs_autofill_utils_set_free_cb,
                             card_type_name);
}
...
init_personal_data (PrefsAutofillDialog *dialog)
{
  ...
  ephy_autofill_storage_get (EPHY_AUTOFILL_FIELD_COUNTRY_CODE,
...
{
  ephy_autofill_storage_get (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_MONTH_M,
                             prefs_autofill_utils_get_combo_box_text_cb,
                             dialog->expdate_month_combo_box_text);

  ephy_autofill_storage_get (EPHY_AUTOFILL_FIELD_CARD_EXPDATE_YEAR_YY,
                             prefs_autofill_utils_get_combo_box_text_cb,
  ...
  ephy_autofill_storage_get (EPHY_AUTOFILL_FIELD_CARD_TYPE_CODE,
                             prefs_autofill_utils_get_combo_box_text_cb,
...
class_init_card_fields (GtkWidgetClass *widget_class)
{
  gtk_widget_class_bind_template_child (widget_class,
                                        PrefsAutofillDialog,
                                        expdate_month_combo_box_text);
  gtk_widget_class_bind_template_child (widget_class,
                                        PrefsAutofillDialog,
                                        expdate_year_combo_box_text);
  gtk_widget_class_bind_template_child (widget_class,
                                        PrefsAutofillDialog,
                                        card_type_combo_box_text);
  gtk_widget_class_bind_template_child (widget_class,
                                        PrefsAutofillDialog,
                                        name_on_card_entry);
  gtk_widget_class_bind_template_child (widget_class,
...
{
  const char *resource_uri = "/org/gnome/epiphany/gtk/prefs-autofill-dialog.ui";
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  gtk_widget_class_set_template_from_resource (widget_class, resource_uri);
