...
prefs_dialog_class_init (PrefsDialogClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, PrefsDialog, lang_listbox);
                           ...
                           PrefsDialog *pd)
{
  ...
  g_file_create_async (js_file, G_FILE_CREATE_NONE, G_PRIORITY_DEFAULT, NULL, js_file_created_cb, NULL);
}
...
static GtkDialog *setup_add_language_dialog (PrefsDialog *dialog);
                     ...
                     const char  *desc)
{
  ...
  for (index = 0; index < len; index++) {
    ...
    widget = gtk_list_box_get_row_at_index (GTK_LIST_BOX (dialog->lang_listbox), index++);
...
language_editor_add_function_buttons (PrefsDialog *dialog)
{
  ...
  gtk_list_box_insert (GTK_LIST_BOX (dialog->lang_listbox), box, -1);
}
...
create_language_section (PrefsDialog *dialog)
{
  ...
  style_context = gtk_widget_get_style_context (dialog->lang_listbox);
  gtk_style_context_add_class (style_context, "frame");
  gtk_list_box_set_header_func (GTK_LIST_BOX (dialog->lang_listbox), hdy_list_box_separator_header, NULL, NULL);
