                      ...
                      gint          new_pos)
{
  ...
  pt_data->notebook = g_object_ref (notebook);
...
{
  g_clear_object (&pt_data->notebook);
  g_free (pt_data);
}
...
remove_arrow_flash (gpointer user_data)
{
  ...
  gtk_style_context_remove_class (gtk_widget_get_style_context (GTK_WIDGET (pt_data->notebook)), "tab-arrow-attention");
  pt_data->arrow_flash_source = 0;
...
present_new_tab (gpointer user_data)
{
  ...
  if (!gtk_widget_get_mapped (GTK_WIDGET (label))) {
    gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (pt_data->notebook)), "tab-arrow-attention");
    pt_data->arrow_flash_source = g_timeout_add (500, remove_arrow_flash, pt_data);
  } else {
    present_tab_data_free (pt_data);
  }
                       ...
                       gboolean      jump_to)
{
  ...
  if (jump_to) {
    ...
    g_idle_add (present_new_tab, pt_data);
  }
