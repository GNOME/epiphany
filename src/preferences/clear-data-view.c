...
G_DEFINE_TYPE (ClearDataView, clear_data_view, EPHY_TYPE_PREFS_DATA_VIEW)
                         ...
                         ClearDataView            *clear_data_view)
{
  ...
  prefs_data_view_set_is_loading (EPHY_PREFS_DATA_VIEW (clear_data_view), FALSE);

  if (!data_list) {
    prefs_data_view_set_has_data (EPHY_PREFS_DATA_VIEW (clear_data_view), FALSE);
  ...
  prefs_data_view_set_has_data (EPHY_PREFS_DATA_VIEW (clear_data_view), TRUE);
...
on_clear_all_clicked (ClearDataView *clear_data_view)
{
  ...
  if (!gtk_tree_model_get_iter_first (clear_data_view->treestore, &top_iter)) {
    gtk_widget_destroy (GTK_WIDGET (clear_data_view));
    return;
  }
                  ...
                  ClearDataView *clear_data_view)
{
  ...
  search_text = prefs_data_view_get_search_text (EPHY_PREFS_DATA_VIEW (clear_data_view));
...
clear_data_view_init (ClearDataView *clear_data_view)
{
  ...
  prefs_data_view_set_is_loading (EPHY_PREFS_DATA_VIEW (clear_data_view), TRUE);
