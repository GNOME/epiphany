                 ...
                 gpointer user_data)
{
  ...
  g_print ("%s(): *** Got search feedback %d, %.1fs\n", __FUNCTION__, g_list_length (self->urls), g_timer_elapsed (self->timer, NULL));

  clear_listbox (self->listbox);

  g_print ("%s(): *** Adding idle loop, %.1fs\n", __FUNCTION__, g_timer_elapsed (self->timer, NULL));
...
add_urls_source (EphyHistoryDialog *self)
{
  ...
  if (!self->num_fetch) {
    self->sorter_source = 0;
    g_print ("%s(): *** Remove (2), %.1fs\n", __FUNCTION__, g_timer_elapsed (self->timer, NULL));
    return G_SOURCE_REMOVE;
  }

  g_print ("%s(): *** Exit and continue, %.1fs\n", __FUNCTION__, g_timer_elapsed (self->timer, NULL));
...
ephy_history_dialog_init (EphyHistoryDialog *self)
{
  ...
  gtk_list_box_set_placeholder (GTK_LIST_BOX (self->listbox), self->spinner);
