...
get_url_from_row (GtkListBoxRow *row)
{
  ...
  url->title = g_strdup (gtk_label_get_text ( GTK_LABEL (gtk_grid_get_child_at (GTK_GRID (grid), 0, 0))));
  url->url = g_strdup (gtk_label_get_text ( GTK_LABEL (gtk_grid_get_child_at (GTK_GRID (grid), 0, 1))));
...
add_urls_source (EphyHistoryDialog *self)
{
  ...
  do {
    ...
    if (ephy_embed_shell_get_mode (shell) != EPHY_EMBED_SHELL_MODE_INCOGNITO) {
      GtkWidget *button = gtk_button_new_from_icon_name ("user-trash-symbolic", GTK_ICON_SIZE_BUTTON);
  ...
  } while (0 && self->urls && g_timer_elapsed (timer, NULL) < GTK_TREE_VIEW_TIME_MS_PER_IDLE / 1000);
