            ...
            EphyHistoryURL    *url)
{
  ...
  /* The CheckButton should be visible only during selection mode */
                               ...
                               EphyHistoryDialog *self)
{
  ...
  if (event->button == GDK_BUTTON_SECONDARY) {
    ...
    /* Primary button toggles a row's selection */
    if (gtk_list_box_row_is_selected (row))
      gtk_list_box_unselect_row (GTK_LIST_BOX (self->listbox), row);
    else
      gtk_list_box_select_row (GTK_LIST_BOX (self->listbox), row);
