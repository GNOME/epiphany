                                       ...
                                       const char          *tag)
{
  ...
  g_set_str (&self->tag_detail_tag, tag);
...
{
  EphyBookmarksDialog *self = EPHY_BOOKMARKS_DIALOG (user_data);
  gboolean tag_detail_has_row = !!gtk_list_box_get_row_at_index (GTK_LIST_BOX (self->tag_detail_list_box), 0);
  GtkListBoxRow *row;
  int idx = 0;
