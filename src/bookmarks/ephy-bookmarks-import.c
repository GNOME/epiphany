                   ...
                   const char *replace)
{
  ...
  if (ptr != NULL) {
    char *after_find = replace_str (ptr + strlen (find), find, replace);
    char *before_find = g_strndup (retval, ptr - retval);
    char *temp = g_strconcat (before_find, replace, after_find, NULL);
    g_free (retval);
