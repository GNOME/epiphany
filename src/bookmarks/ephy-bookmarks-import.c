...
parser_data_new ()
{
  ...
  data->urls_table = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, (GDestroyNotify)g_ptr_array_free);
    ...
    while (*names) {
      if (strcmp (*names, "HREF") == 0) {
        ...
        if (g_hash_table_lookup_extended (data->urls_table, *values, NULL, (gpointer *)&old_tags))
          g_ptr_array_extend (tags, old_tags, NULL, NULL);
                                 ...
                                 GError               **error)
{
  ...
  for (guint i = 0; i < data->urls->len; i++) {
    const char *guid = ephy_bookmark_generate_random_id ();
