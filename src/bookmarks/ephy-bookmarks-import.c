...
gchar *replace_str (const gchar *src,
                    const gchar *find,
                    const gchar *replace)
{
                   ...
                   GError              **error)
{
  ...
  if (0 == strcmp (element_name, "A")) {
    data->read_title = (gboolean *)TRUE;
    while (*names) {
      if (0 == strcmp (*names, "HREF")) {
        g_ptr_array_add (data->urls, (gpointer)g_strdup (*values));
      }
      if (0 == strcmp (*names, "ADD_DATE")) {
                                 ...
                                 GError               **error)
{
  ...
  if (g_markup_parse_context_parse (context, buf, strlen (buf), &my_error) == FALSE) {
...
out:
  ...
  /*ephy_bookmarks_manager_create_tag (manager, "Favorites"); */
  /*g_sequence_append (tags, "Favorites"); */
