  ...
  if (strcmp (element_name, "A") == 0) {
    ...
    while (*names) {
      if (strcmp (*names, "HREF") == 0) {
        GPtrArray *tags;
        char *tag = g_queue_peek_head (data->tags_stack);

        if (!g_hash_table_lookup_extended (data->urls_table, *values, NULL, &tags))
                 ...
                 GError              **error)
{
  ...
  if (strcmp (element_name, "DL") == 0)
    g_queue_pop_head (data->tags_stack);
}
          ...
          GError              **error)
{
  ...
  if (data->read_tag) {
    g_queue_push_head (data->tags_stack, (gpointer)g_strdup (text));
    g_ptr_array_add (data->tags, (gpointer)g_strdup (text));
  }
  if (data->read_title)
                                 ...
                                 GError               **error)
{
  ...
  for (guint i = 0; i < data.urls->len; i++) {
    ...
    tags = g_sequence_new (g_free);
    g_hash_table_lookup_extended (data.urls_table, url, NULL, &val);
