...
static gboolean
dialog_passwords_import_from_chrome (GtkDialog *dialog)
{
  GtkWidget *import_info_dialog;
  g_autoptr (GError) error = NULL;
  g_autofree gchar *filename;
  ...
  imported = ephy_password_import_from_chrome (manager, filename, TRUE, &error);
  import_info_dialog = gtk_message_dialog_new (GTK_WINDOW (dialog),
                                               GTK_DIALOG_MODAL,
                                               imported ? GTK_MESSAGE_INFO : GTK_MESSAGE_WARNING,
                                               GTK_BUTTONS_OK,
...
static gboolean
dialog_passwords_import_from_chromium (GtkDialog *dialog)
{
  GtkWidget *import_info_dialog;
  g_autoptr (GError) error = NULL;
  g_autofree gchar *filename;
  ...
  imported = ephy_password_import_from_chrome (manager, filename, FALSE, &error);
  import_info_dialog = gtk_message_dialog_new (GTK_WINDOW (dialog),
                                               GTK_DIALOG_MODAL,
                                               imported ? GTK_MESSAGE_INFO : GTK_MESSAGE_WARNING,
                                               GTK_BUTTONS_OK,
                            ...
                            GtkComboBox *combo_box)
{
  ...
  if (response == GTK_RESPONSE_OK) {
    ...
    switch (active) {
      case 0:
        imported = dialog_passwords_import_from_chrome (dialog);
        break;
      case 1:
        imported = dialog_passwords_import_from_chromium (dialog);
        break;
      default:
        g_assert_not_reached ();
    }
