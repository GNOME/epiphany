                             ...
                             GdkEventKey *event)
{
  ...
  if (gtk_window_get_focus (GTK_WINDOW (widget)) != GTK_WIDGET (view)) {
    /* WebView not focused, handle the key event in GTK's usual way:
     * 1) Send the key event to the GtkWindow to activate any shortcuts or
     *    accelerators the window has
     * 2) If there were no accelerators found to handle the event, then send
     *    the key event to the focused widget from which it will chain up
     *    until it is handled
     *
     * From https://developer.gnome.org/gtk3/stable/chap-input-handling.html
     */
  ...
  /* WebView is focused, handle the key event differently from GTK's usual way:
