...
ephy_web_extension_proxy_dispose (GObject *object)
{
  ...
  if (web_extension->autofill_signal_id > 0) {
    g_dbus_connection_signal_unsubscribe (web_extension->connection,
                                          web_extension->autofill_signal_id);
    web_extension->autofill_signal_id = 0;
  }
...
ephy_web_extension_proxy_class_init (EphyWebExtensionProxyClass *klass)
{
  ...
  /**
   * EphyWebExtensionProxy::autofill:
   * @web_extension: the #EphyWebExtensionProxy
   * @page_id: the identifier of the web page created
...
autofill_signal_received_cb (GDBusConnection *connection,
                             ...
                             gpointer user_data)
{
  EphyWebExtensionProxy *web_extension = user_data;
  ...
  g_signal_emit (web_extension, signals[AUTOFILL_SIGNAL], 0,
                 page_id, selector, is_fillable_element, has_personal_fields, has_card_fields,
                 element_x, element_y, element_width, element_height);
}
...
void
ephy_web_extension_proxy_autofill (EphyWebExtensionProxy *web_extension,
                                   guint64 page_id,
                                   const char *selector,
                                   int fill_choice)
{
