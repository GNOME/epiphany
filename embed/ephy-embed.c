...
{
  gboolean show_document, show_paned;
  ...
  gtk_widget_set_visible (GTK_WIDGET (embed->paned), show_paned);
  if (embed->document_view)
    gtk_widget_set_visible (embed->document_view, show_document);
}
...
static gboolean
document_download_decide_destination_cb (WebKitDownload *download,
                             ...
                             WebKitDownload *download)
{
  if (embed->mode == EPHY_EMBED_MODE_DOCUMENT) {
    ...
    g_signal_connect (download, "decide-destination", G_CALLBACK (document_download_decide_destination_cb), embed);
  }
}
