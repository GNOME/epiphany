...
filter_info_needs_fetching (const FilterInfo *self)
{
  g_assert (self);
                ...
                FilterInfo                   *self)
{
  ...
  g_autoptr(WebKitUserContentFilter) wk_filter =
    webkit_user_content_filter_store_save_finish (self->manager->store,
                                                  result,
                                                  &error);
  if (wk_filter) {
                             ...
                             GFile      *json_file)
{
  ...
  if (!filter_info_needs_fetching (self) && self->found &&
                ...
                FilterInfo                   *self)
{
  ...
  g_autoptr(GError) error = NULL;
  g_autoptr(WebKitUserContentFilter) wk_filter =
      webkit_user_content_filter_store_load_finish (self->manager->store,
                                                    result,
                                                    &error);
  ...
  if (!filter_info_needs_fetching (self))
    return;
...
static char*
wk_saved_filters_dir (EphyFiltersManager *manager)
{
...
ephy_filters_manager_constructed (GObject *object)
{
  ...
  g_autofree char *saved_filters_dir = wk_saved_filters_dir (manager);
  ...
  manager->update_timeout_id =
    g_timeout_add_seconds (ADBLOCK_FILTER_UPDATE_FREQUENCY,
