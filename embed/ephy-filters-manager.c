...
filter_info_get_identifier (FilterInfo *self)
{
  g_assert (self);
  if (!self->identifier)
    self->identifier = g_compute_checksum_for_string (G_CHECKSUM_SHA256, self->source_uri, -1);
                          ...
                          GError     **error)
{
  ...
  if (file_type != G_FILE_TYPE_REGULAR) {
    ...
    g_set_error (error, G_IO_ERROR, error_code, "%s: %s", message, sidecar_file_path);
    return FALSE;
  }
                          ...
                          EphyFiltersManager *manager)
{
  ...
  if (g_hash_table_contains (manager->filters, identifier)) {
    LOG ("Filter %s in current filter set, not removing.", identifier);
  } else {
    ...
    webkit_user_content_filter_store_remove (filter->manager->store,
                                             ...
                                             NULL);
    LOG ("Filter %s missing from current set, removal scheduled.", identifier);
  }
}
