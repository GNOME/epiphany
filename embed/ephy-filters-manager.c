...
filter_info_needs_updating_from_source (const FilterInfo *self)
{
   ...
   */
  if (self->local) {
    g_autoptr (GDateTime) modification_date = NULL;
    ...
    modification_date = g_file_info_get_modification_date_time (info);
    modification_time = g_date_time_to_unix (modification_date);

    return (modification_time > 0) && ((gulong)modification_time > self->last_update);
  }
