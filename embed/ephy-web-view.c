                                  ...
                                  WebKitPermissionRequest *request)
{
  ...
  /* FIXME: do we want to save this decision persistently? */
                                            ...
                                            WebKitWebsiteDataAccessPermissionRequest *decision)
{
  ...
  char *message;
  char *markup;
  const char *requesting_domaing;
  const char *current_domain;

  info_bar = gtk_info_bar_new_with_buttons (_("No"), GTK_RESPONSE_NO,
                                            _("Yes"), GTK_RESPONSE_YES,
                                            NULL);

  box = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);

  requesting_domaing = webkit_website_data_access_permission_request_get_requesting_domain (decision);
  current_domain = webkit_website_data_access_permission_request_get_current_domain (decision);
  message = g_strdup_printf (_("Do you want to allow “%s“ to use cookies while browsing “%s“?"), requesting_domaing, current_domain);
  ...
  message = g_strdup_printf (_("This will allow “%s“ to track your activity"), requesting_domaing);
