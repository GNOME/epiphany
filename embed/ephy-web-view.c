...
static void
on_unresponsive_dialog_response (AdwMessageDialog2 *dialog,
                                 const char       *response,
                                 EphyWebView      *web_view)
{
  ...
  g_clear_weak_pointer (&web_view->unresponsive_process_dialog);
}
...
unresponsive_process_timeout_cb (gpointer user_data)
{
  ...
  g_set_weak_pointer (&web_view->unresponsive_process_dialog,
                      adw_message_dialog2_new (_("Page Unresponsive"), NULL));

  adw_message_dialog2_format_body (ADW_MESSAGE_DIALOG2 (web_view->unresponsive_process_dialog),
                                  _("The current page “%s” is not responding"),
                                  ephy_web_view_get_address (web_view));

  adw_message_dialog2_add_responses (ADW_MESSAGE_DIALOG2 (web_view->unresponsive_process_dialog),
                                    "wait", _("_Wait"),
                                    "stop", _("Force _Stop"),
                                    NULL);
  ...
  adw_dialog_present (ADW_DIALOG (web_view->unresponsive_process_dialog),
                      GTK_WIDGET (gtk_widget_get_root (GTK_WIDGET (web_view))));
                                      ...
                                      gpointer     user_data)
{
  ...
  if (web_view->unresponsive_process_dialog && responsive) {
    ...
    if (ADW_IS_DIALOG (web_view->unresponsive_process_dialog)) {
      adw_dialog_close (ADW_DIALOG (web_view->unresponsive_process_dialog));
      g_clear_weak_pointer (&web_view->unresponsive_process_dialog);
    }
  }
