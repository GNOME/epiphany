                      ...
                      WebKitURISchemeRequest *request)
{
  ...
  pdf_request->web_view = NULL;
  pdf_request->cancellable = g_cancellable_new ();
  pdf_request->load_changed_id = 0;
...
{
  if (request->download) {
    g_signal_handler_disconnect (request->download, request->download_completed_id);
    g_signal_handler_disconnect (request->download, request->download_error_id);
                       ...
                       EphyPdfRequest *self)
{
  ...
  if (g_strcmp0 ("application/pdf", ephy_download_get_content_type (download)) == 0) {
    ...
    g_file_load_contents_async (file, NULL, pdf_file_loaded, self);
  } else {
...
ephy_pdf_request_start (EphyPdfRequest *request)
{
  ...
  request->download_completed_id = g_signal_connect (request->download, "completed",
                                                     G_CALLBACK (download_completed_cb), request);
  request->download_error_id = g_signal_connect (request->download, "error",
                                                 G_CALLBACK (download_errored_cb), request);
}
