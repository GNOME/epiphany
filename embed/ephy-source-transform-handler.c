                      ...
                      EphySourceTransformRequest *request)
{
  ...
  EphySourceTransformHandlerPrivate *priv = ephy_source_transform_handler_get_instance_private (EPHY_SOURCE_TRANSFORM_HANDLER (handler));
  g_autofree guchar *source = NULL;
  char *html;
  gsize length;
  ...
  priv->title = g_strdup (webkit_web_resource_get_uri (resource));
...
embed_is_displaying_matching_uri (EphyEmbed *embed,
                                  SoupURI   *uri)
{
  g_autoptr (SoupURI) view_uri = NULL;
...
ephy_source_transform_request_start (EphySourceTransformRequest *request)
{
  ...
  g_autofree char *decoded_fragment = NULL;
  ...
  if (!soup_uri || !soup_uri->fragment) {
    ...
    g_autoptr (GError) error = g_error_new (WEBKIT_NETWORK_ERROR,
...
const char *
ephy_source_transform_handler_get_uri (EphySourceTransformHandler *handler)
{
