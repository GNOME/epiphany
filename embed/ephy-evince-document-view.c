...
ephy_evince_document_view_constructed (GObject *object)
{
  ...
  self->ev_view = ev_view_new ();
  g_signal_connect (self->ev_view, "external-link", G_CALLBACK (view_external_link_cb), self);
  gtk_container_add (GTK_CONTAINER (scrolled_window), self->ev_view);
  gtk_box_pack_start (GTK_BOX (self), scrolled_window, TRUE, TRUE, 0);

  self->model = ev_document_model_new ();
  ev_view_set_model (EV_VIEW (self->ev_view), self->model);
